...
const LeadershipComponent = () => {
  ...
  const zbczLeadership = {
    executiveCouncil: [{ name: "Archbishop (Dr) Henry Ziwerere", title: "President & Founder", region: "Head Office", image: "/mb4.jpeg", bio: "Archbishop (Dr) Henry Ziwerere is the founder and Inaugural President of the Zion Bishop's Conference of Zimbabwe International (ZBCZI).", achievements: ["Inaugural President & Founder of ZBCZI", "Founded the conference on 31 August 2022", "Led the official launch in Bulawayo, 24 May 2024", "Provides doctrinal and strategic oversight"], specialization: "Church Unity & Leadership", contact: "info@zbcz.org" }],
    seniorLeadership: [
      { name: "Rev Adonija Muzondiona", title: "Vice President", region: "ZBCZI", image: "/user.png", bio: "Rev Adonija Muzondiona serves as a Vice President of ZBCZI, supporting executive leadership and the coordination of member churches.", achievements: ["Vice President of ZBCZI", "Supports executive leadership", "Coordination of member churches"], specialization: "Executive Leadership", contact: "info@zbcz.org" },
      { name: "High Priest Peter Mpofu Chatira", title: "Vice President", region: "ZBCZI", image: "/user.png", bio: "High Priest Peter Mpofu Chatira serves as a Vice President of ZBCZI, supporting the conference's executive leadership and ministry.", achievements: ["Vice President of ZBCZI", "Supports executive leadership", "Pastoral and ministry oversight"], specialization: "Executive Leadership", contact: "info@zbcz.org" },
      { name: "Archbishop Samuel Magutakuwona", title: "Secretary General", region: "ZBCZI", image: "/user.png", bio: "Archbishop Samuel Magutakuwona serves as the Secretary General of ZBCZI, responsible for administration and governance.", achievements: ["Secretary General of ZBCZI", "Administrative oversight", "Governance and coordination"], specialization: "Administration & Governance", contact: "info@zbcz.org" }
    ],
    patron: [
      { name: "Archbishop (Prof) David Mureri", title: "Patron", region: "ZBCZI", image: "/user.png", bio: "Archbishop (Prof) David Mureri is the Patron of the Zion Bishop's Conference of Zimbabwe International (ZBCZI), offering guidance and counsel to the conference.", achievements: ["Patron of ZBCZI", "Provides guidance and counsel", "Supports the vision of unity"], specialization: "Patronage & Counsel", contact: "info@zbcz.org" }
    ],
    publicRelations: [
      { name: "Archbishop V. Chitumwa", title: "Head of Public Relations", region: "ZBCZI", image: "/user.png", bio: "Archbishop V. Chitumwa is the Head of Public Relations for ZBCZI. Direct line: 0774 170 347.", achievements: ["Head of Public Relations", "Media and communications", "Public engagement"], specialization: "Public Relations", contact: "info@zbcz.org" },
      { name: "Bishop Mlambo", title: "Vice Public Relations", region: "ZBCZI", image: "/user.png", bio: "Bishop Mlambo is the Vice Public Relations officer for ZBCZI. Direct line: 0773 451 060.", achievements: ["Vice Public Relations", "Supports media and communications", "Public engagement"], specialization: "Public Relations", contact: "info@zbcz.org" }
    ]
  };

  const sections = [
    { id: 'executive', label: 'President', icon: Crown },
    { id: 'senior', label: 'Senior', icon: Users },
    { id: 'patron', label: 'Patron', icon: Medal },
    { id: 'pr', label: 'Public Relations', icon: UserCircleCheck },
    { id: 'structure', label: 'Org Chart', icon: Buildings }
  ];

  const getData = () => activeSection === 'executive' ? zbczLeadership.executiveCouncil : activeSection === 'senior' ? zbczLeadership.seniorLeadership : activeSection === 'patron' ? zbczLeadership.patron : activeSection === 'pr' ? zbczLeadership.publicRelations : [];
    ...
    <div className="min-h-screen bg-cream-50">
      ...
      <section className="relative py-20 sm:py-28 overflow-hidden pt-32 sm:pt-36">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="font-heading text-4xl sm:text-5xl font-bold text-white mb-5">ZBCZI <span className="text-gold-400">Leadership</span></h1>
          <p className="text-cream-200/70 text-lg max-w-2xl mx-auto mb-10">Dedicated servants leading in unity and faith</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
            {[{ n: "2022", l: "Founded", icon: Crown }, { n: "2024", l: "Launched", icon: Buildings }, { n: "Christ", l: "Our Head", icon: Church }, { n: "Unity", l: "In Faith", icon: Globe }].map((s, i) => (
          ...
          {activeSection === 'structure' ? (
            <div className="bg-white rounded-3xl shadow-card p-8 sm:p-12 border border-cream-200 max-w-2xl mx-auto">
              ...
              <div className="space-y-5 text-center">
                <div className="inline-block bg-cream-200 text-maroon-700 px-6 py-3 rounded-xl shadow-md font-bold text-sm"><Medal size={18} className="inline mr-2" />Patron</div>
                <div><CaretDown size={20} className="mx-auto text-cream-400" /></div>
                <div className="inline-block bg-maroon-500 text-white px-6 py-3 rounded-xl shadow-lg font-bold text-sm"><Crown size={18} className="inline mr-2" />President & Founder</div>
                <div><CaretDown size={20} className="mx-auto text-cream-400" /></div>
                <div className="flex justify-center gap-3 flex-wrap">
                  {['Vice President', 'Vice President', 'Secretary General'].map((t, i) => <div key={i} className="bg-gold-400 text-maroon-900 px-5 py-2.5 rounded-xl shadow-md text-sm font-bold">{t}</div>)}
                </div>
                <div><CaretDown size={20} className="mx-auto text-cream-400" /></div>
                <div className="flex justify-center gap-3 flex-wrap">
                  {['Head of Public Relations', 'Vice Public Relations'].map((t, i) => <div key={i} className="bg-forest-500 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md">{t}</div>)}
                </div>
                <div><CaretDown size={20} className="mx-auto text-cream-400" /></div>
                <div className="inline-block bg-cream-200 text-maroon-700 px-6 py-3 rounded-xl text-sm font-bold"><Church size={18} className="inline mr-2" />Member Churches</div>
              </div>
            </div>
          ) : (
              ...
              {getData().map((l, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="mb-4"><h4 className="font-semibold text-maroon-700 mb-2 text-sm flex items-center gap-1.5"><Star size={14} weight="fill" className="text-gold-500" />Role & Responsibilities</h4>
