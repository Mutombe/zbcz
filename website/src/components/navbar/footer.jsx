...
import { MapPin, Phone, Envelope, Globe, FacebookLogo, XLogo, InstagramLogo, YoutubeLogo, Church, Users, Heart, ArrowRight, Code, Cross, Buildings } from '@phosphor-icons/react';
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div className="col-span-2 lg:col-span-1">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div><h3 className="font-heading text-lg sm:text-xl font-bold text-gold-400">ZBCZI</h3><p className="text-[10px] sm:text-xs text-cream-300/50">Est. 2022</p></div>
            </div>
            <p className="text-xs sm:text-sm text-cream-300/60 leading-relaxed mb-5">Uniting indigenous Zion Churches under one Head, Jesus Christ. Building God's kingdom through unity, love, and faithful service.</p>
            <div className="glass rounded-2xl p-3.5 sm:p-4 mb-5">
              <div className="space-y-2">
                {[{ icon: Cross, label: "Founded", val: "31 Aug 2022", c: "text-gold-400" }, { icon: Buildings, label: "Launched", val: "24 May 2024", c: "text-forest-300" }, { icon: MapPin, label: "Launched in", val: "Bulawayo", c: "text-cream-300" }].map((s, i) => (
            ...
            <div className="flex gap-2">
              <a href="https://www.facebook.com/share/1CaKy6zh4y/" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="glass rounded-xl p-2 sm:p-2.5 hover:bg-gold-400 hover:text-maroon-900 transition-all duration-300 text-cream-300/60 flex items-center gap-2 px-3"><FacebookLogo size={16} weight="fill" /><span className="text-xs font-semibold">Follow us on Facebook</span></a>
            </div>
          </div>
          ...
          {/* Col 3 - Public Relations */}
          <div className="hidden sm:block">
            <h4 className="font-semibold mb-4 text-gold-400 text-xs sm:text-sm flex items-center gap-2"><Phone size={12} weight="fill" />Public Relations</h4>
            <div className="space-y-2">
              {[{ name: "Archbishop V. Chitumwa", role: "Head of Public Relations", tel: "0774170347", disp: "0774 170 347" }, { name: "Bishop Mlambo", role: "Vice Public Relations", tel: "0773451060", disp: "0773 451 060" }].map((o, i) => (
                <div key={i} className="glass rounded-lg sm:rounded-xl px-3 sm:px-4 py-2.5">
                  <div className="text-cream-200/80 text-xs sm:text-sm font-medium">{o.name}</div>
                  <div className="text-[10px] sm:text-xs text-cream-300/50 mb-1">{o.role}</div>
                  <a href={`tel:+263${o.tel.slice(1)}`} className="text-[11px] sm:text-xs text-gold-400/80 hover:text-gold-400 transition-colors flex items-center gap-1"><Phone size={11} weight="fill" />{o.disp}</a>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Col 4 - Contact */}
          <div>
            <h4 className="font-semibold mb-4 text-gold-400 text-xs sm:text-sm flex items-center gap-2"><Buildings size={12} weight="fill" />Head Office</h4>
            <div className="glass rounded-xl sm:rounded-2xl p-4 sm:p-5 space-y-3 text-xs sm:text-sm">
              <div className="flex items-start gap-2.5"><MapPin size={14} className="text-cream-400/50 mt-0.5 flex-shrink-0" /><span className="text-cream-300/60">ZBCZI Head Office<br/>Margolis Plaza, 2nd Floor, Room 201A</span></div>
              <div className="flex items-center gap-2.5"><Phone size={14} className="text-cream-400/50 flex-shrink-0" /><a href="tel:+263774170347" className="text-cream-300/60 hover:text-gold-400 transition-colors">0774 170 347</a></div>
              ...
              <div className="flex items-center gap-2.5"><FacebookLogo size={14} className="text-cream-400/50 flex-shrink-0" /><a href="https://www.facebook.com/share/1CaKy6zh4y/" target="_blank" rel="noopener noreferrer" className="text-cream-300/60 hover:text-gold-400 transition-colors">Facebook Page</a></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-[10px] sm:text-xs text-cream-300/40 text-center sm:text-left">&copy; {currentYear} Zion Bishop's Conference of Zimbabwe International. All rights reserved.</p>
